...
cat > tmp/Dockerfile << EOF
        ...
        ENV HDFS_NODEMANAGER_USER root
...
master_id=$(docker run -p 9870:9870 -d --net caochong --name caochong-master -h master --network-alias=master caochong-$MODE)
#echo ${master_id:0:12} > hosts
echo "master" > hosts
for i in $(seq $((N-1)));
do
    container_id=$(docker run -d --net caochong --name caochong-slave-$i -h slave$i --network-alias=slave$i caochong-$MODE)
